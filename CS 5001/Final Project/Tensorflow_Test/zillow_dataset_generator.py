...
def define_variables_and_lists():
    ...
    days_on_zillow = []
    zpids = []
    return max_results, num_results, total_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids



def get_0b_1b_sort_priority_score_doz_1(max_results, num_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids, total_results):
    print("Obtaining all listings that contain 1 bedroom and 1 bathroom, sorted by priority score")
    num_results = 0
    # Set up the Zillow API endpoint and parameters
    url = "https://zillow56.p.rapidapi.com/search"
    query_params = {"location": "New York, NY",
                    "status": "forRent", "sort": "priorityScore", "beds_max": "0", "baths_max": "1", "doz": "1"}
    ...
    tota_result_count = data['totalResultCount']
    while num_results < tota_result_count:
        response = requests.get(url, headers=headers, params=query_params)
    ...
    # Check if the response was successful
        if response.status_code == 200:
            ...
            data = json.loads(response.content)
            tota_result_count = data['totalResultCount']
            print("Amount of found listings: ", tota_result_count)
            while num_results < tota_result_count:
    ...
    return df, df2
def get_1b_1b_sort_priority_score_doz_1(max_results, num_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids, total_results):
    print("Obtaining all listings that contain 1 bedroom and 1 bathroom, sorted by priority score")
    num_results = 0
    # Set up the Zillow API endpoint and parameters
    url = "https://zillow56.p.rapidapi.com/search"
    query_params = {"location": "New York, NY",
                    "status": "forRent", "sort": "priorityScore", "beds_min": "1", "baths_min" : "1","beds_max": "1", "baths_max": "1", "doz": "1"}
    ...
    return df, df2
def get_2b_1b_sort_priority_score_doz_1(max_results, num_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids, total_results):
    print("Obtaining all listings that contain 1 bedroom and 1 bathroom, sorted by priority score")
    num_results = 0
    # Set up the Zillow API endpoint and parameters
    url = "https://zillow56.p.rapidapi.com/search"
    query_params = {"location": "New York, NY",
                    "status": "forRent", "sort": "priorityScore", "beds_min": "2", "baths_min" : "1","beds_max": "2", "baths_max": "1", "doz": "1"}
    ...
    return df, df2
def get_3b_1b_sort_priority_score_doz_1(max_results, num_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids, total_results):
    print("Obtaining all listings that contain 1 bedroom and 1 bathroom, sorted by priority score")
    num_results = 0
    # Set up the Zillow API endpoint and parameters
    url = "https://zillow56.p.rapidapi.com/search"
    query_params = {"location": "New York, NY",
                    "status": "forRent", "sort": "priorityScore", "beds_min": "3", "baths_min" : "1","beds_max": "3", "baths_max": "1", "doz": "1"}
...
def main():
    ...
    max_results, num_results, total_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids = define_variables_and_lists()
    #zillow api is limited to 800 results per search even with using pagination techniques. 
    # Splitting up the api calls by number of bedrooms and bathrooms as well as the way that its sorted.
    df, df2 = get_0b_1b_sort_priority_score_doz_1(max_results, num_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids, total_results)
    df, df2 = get_1b_1b_sort_priority_score_doz_1(max_results, num_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids, total_results)
    df, df2 = get_2b_1b_sort_priority_score_doz_1(max_results, num_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids, total_results)
    df, df2 = get_3b_1b_sort_priority_score_doz_1(max_results, num_results, results_per_page, current_page, photo_urls, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, zpids, total_results)
   # save_dataframe_to_csv(df, df2, zpids, listing_prices, bathrooms, bedrooms, living_areas, days_on_zillow, photo_urls)
